<template>
  <div class="search-detail">
    <div class="search-container">
      <div class="search-hd">
        <el-select v-model="keyVal" class="search-box" clearable filterable remote placeholder="请输入歌名、歌词、歌手或专辑"
          :remote-method="remoteMethod" :fit-input-width="true" :loading="loading" loading-text="搜索中..."
          @focus="handleFocus" @clear="clearVal" @keyup.enter="enterHandler">
          <el-option-group v-for="list in suggestInfo" :key="listType[list.label]" :label="listType[list.label]">
      ...
      <div class="search-main">
        ...
        <div class="search-tab">
          <div class="tab-item" v-for="item in typeList" :key="item.k" :class="{ active: type == item.k }"
            @click="selectType(item)">{{ item.v }}</div>
        </div>
        <div class="search-list" v-if="total || loading">
          ...
          <album-list class="albums" :albumList="list" :loading="loading" :num="limit"
            v-else-if="type == '10'"></album-list>
          <search-artist :list="list" :loading="loading" v-else-if="type == '100'"></search-artist>
          <play-list class="play" :playList="list" :loading="loading" :num="limit"
            v-else-if="type === '1000'"></play-list>
