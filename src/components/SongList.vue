<template>
  <div class="songs-list-main">
        ...
        <div :class="isCurSong(item, index)" v-for="(item, index) in list" :key="item.id">
          ...
          <div class="columnSinger songlist-singer">
            ...
            <router-link :to="{ path: '/singer', query: { id: author.id } }" class="song_name"
              v-for="(author, k) in item.singer" :key="author.name">{{ k !== 0 ? ' / ' + author.name : author.name
              }}</router-link>
          </div>
    ...
    <div class="pagination" v-if="isShowPagination">
      <el-pagination @current-change="currentChange" :page-size="pageSize" :current-page.sync="currentPage"
        layout="prev, pager, next" :total="songList.length">
      </el-pagination>
    </div>
  </div>
</template>
